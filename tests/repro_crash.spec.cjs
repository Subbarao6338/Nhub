...
test('Dev Hub should not crash when opened', async ({ page }) => {
  ...
  const errorMsg = page.locator('text=Nature hit a snag');
  ...
  const tabs = ['Base64', 'JSON Formatter', 'Diff Viewer', 'JWT Decoder', 'Cron Helper', 'SQL Formatter', 'Regex Tester', 'UUID Gen', 'URL Tool', 'YAML Conv', 'Minifier', 'XML ↔ JSON', 'XML Formatter', 'JSON to TS'];
